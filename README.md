...
- RGB/DVS Fusion：正在开发；未完成的功能继续保持不可点击。
...
Hikrobot `MV-CU050-90UC` 已完成真实 MVS 冒烟测试：曝光读取/写回、连续采集、停止、关闭和 SDK 释放均通过。公开仓库不记录真实设备序列号；详细测试记录位于 `docs/diagnostics/`。
...
- `config/local.json`、MEX、构建产物和本机输出必须保持未跟踪状态。
- 厂商 SDK / Runtime 的再分发需遵循对应厂商许可；公开仓库不应新增未经确认授权的 DLL、LIB、EXE。
- 真实设备序列号、用户名、本机绝对路径等信息不要写入公开文档或配置。
- `feature/reused-frame-fusion` 为 Fusion 开发分支；`main` 保持可运行主线。
